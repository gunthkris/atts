...
while (True):
	...
	if (len(leftcnts) > 0) :
		...
		((x, y), radius) = cv2.minEnclosingCircle(leftc)
		M = cv2.moments(leftc)
		leftcenter = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
		if radius > 10:
		# draw the circle and centroid on the frame,
		# then update the list of tracked points
			cv2.circle(leftFrame, (int(x), int(y)), int(radius),(0, 255, 0), 2)
			cv2.circle(leftFrame, leftcenter, 5, (0, 255, 255), -1)

	if (len(rightcnts) > 0) :
		rightc = max(rightcnts, key=cv2.contourArea)
		((x, y), radius) = cv2.minEnclosingCircle(rightc)
		M = cv2.moments(rightc)
		rightcenter = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
		# only proceed if the radius meets a minimum size
		if radius > 10:
			cv2.circle(rightFrame, (int(x), int(y)), int(radius),(0, 255, 255), 2)
			cv2.circle(rightFrame, rightcenter, 5, (0, 0, 255), -1)
	...
	for i in range(1, len(pts)):
		...
		cv2.line(leftFrame, pts[i - 1], pts[i], (0, 255, 0), thickness)
